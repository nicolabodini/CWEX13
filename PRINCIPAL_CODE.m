...
losv=transpose(losv);
% Radial wind speed dispersion (m/s) - this is the standard deviation
...
while i1 < m % m % m = number of timesteps in the considered day
     ...
     switch flag
      ...
      if (i2 - i1) > 25
          ...
          % Quantile plots: is it reasonable to assume normal distribution
          % in estimating stdev with MAD method?
          %figure()
          %qqplot(losvReshape)
          %figure()
          %histogram(losvReshape)
          ...
          rdispFree2D = (1./abs(inpaint_nans(rdispFree2D))).^2; % weigths are inverse of losv dispersion^2
          ...
          for iR = rInd % loop across different range gates
              ...
              weightsGate = 1./rdispGate.^2;  % weights: inverse of losv dispersion
              ...
              for itent=1:ntent % 4 fitting tries - with different first-guess
                  ...
                  b0{1} = [phi0 u0];
                  % 4 wakes model
                  b0{2} = [b0{1} a0(1) a0(2) a0(3) a0(4) mu0(1) mu0(2) mu0(3) mu0(4) 0.25 0.25 0.25 0.25];
                  ...
                  for iMdl = 1:2 % loop across the 2 models (no-wake, 4 wakes)
                      ...
                      % estimates of the parameter just found
                      losvHat(:,iMdl) = mdlnowgts(bHat{itent,iMdl},yG(i1:i2,iR)); 
                      % Calculate residuals
                      res(:,iMdl)=losvHat(:,iMdl)-losvGate(:,1);
                      % Mean squared error
                      mse(itent,iMdl) = (nansum(weightsGate(:,1).*res(:,iMdl).^2))/(nansum(weightsGate));
                      % Norm of the residuals
                      ss(itent,iMdl) = sum(weightsGate(:,1).*res(:,iMdl).^2);
                      % Covariance matrix
                      ...
                      % Linear correlation coefficient (Pearson) SQUARED - R2
                      rho(itent,iMdl) = (corr(losvGate,losvHat(:,iMdl))).^2;
                  end
